...
var BINDINGS={accelerate:gamejs.event.K_UP, 
              brake:gamejs.event.K_SPACE,
              reverse:gamejs.event.K_DOWN,
...
var DynamicBoxProp = function(pars){
    /*
   static rectangle shaped prop
    ...
    */
    ...
    bdef.position=new box2d.b2Vec2(pars.position[0], pars.position[1]);
    bdef.angle=0;
    bdef.type=box2d.b2_dynamicBody;
    //bdef.fixedRotation=true;
    this.body=b2world.CreateBody(bdef);
...
Car.prototype.update=function(msDuration){
        ...
        var base_vect=[0, 0]; //vector pointing in the direction force will be applied to a wheel ; relative to the wheel.
        ...
        else if(this.accelerate==ACC_REVERSE && (this.getSpeedKMH() < this.max_speed)){
        ...
        if( (this.getSpeedKMH()<5) &&(this.accelerate==ACC_NONE)){
            this.setSpeed(0);
        }
...
function main(){
    ...
    function tick(msDuration) {
        ...
        if(KEYS_DOWN[BINDINGS.steer_right]){
            ...
            display.blit(font.render('RIGHT: '+parseInt(msDuration)), [225, 85]);
        }else if(KEYS_DOWN[BINDINGS.steer_left]){
            any_key = true;
            command += '<';

            display.blit(font.render('LEFT: '+parseInt(msDuration)), [225, 115]);
        }
